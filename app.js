...
function renderGift(doc) {
  ...
  div.innerHTML = `
    ...
    <p class="mt-2">Статус: <span class="font-bold">${data.reserved ? 'Зарезервировано' : 'Не зарезервировано'}</span></p>
    <button class="mt-2 px-4 py-2 text-white ${data.reserved ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'} rounded" ${data.reserved ? 'disabled' : ''}>
